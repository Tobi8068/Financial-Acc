...
export function SuppliersTable({ filters, searchQuery, onClickView }: SuppliersTableProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteItemId, setDeleteItemId] = useState<string | null>(null);

  const { data, totalPages, totalItems, itemsPerPage } = useSuppliersData(
    currentPage,
    filters,
    searchQuery
  );
          ...
          <TableBody>
            {data.length !== 0 && data.map((item) => (
              ...
              >
                ...
                <TableCell className="font-medium">{item.billingAddress}</TableCell>
                <TableCell className="font-medium">{item.shippingAddress}</TableCell>
                <TableCell>
                  <Popover>
